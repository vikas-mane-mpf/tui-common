 define(["jquery",
         "backbone",
         "underscore",
         "text!components/statusDropdown/StatusDropdownTpl.html"
     ],
         ...
         var View = Backbone.View.extend({
            ...
            events: {
             ...
             render : function(options){
                ...
                this.$el.checkbox( this.status === 'active'  ? 'check' : 'uncheck');
             ...
             createOptions: function(isChecked) {
                ...
                $(menuHead).html('Subscription');
                $(menuBox).addClass('menuOptions');
                $(menuBox).append(menuHead);
                var inactiveMsg = 'Are you sure you want to Inactive?';
                var activeMsg = 'Are you sure you want to Active';
                //var exportMsg = 'Unsupported Action. You’re trying to unsubscribe to a segment that is currently exported to DSPs. Please turn off export before unsubscribing';

                $(menuBox).append(isChecked ? inactiveMsg : activeMsg);

                //Inactive to Active then this loop will execute
                if(!isChecked) {
             ...
             update : function(status, iconClass){
               ...
               this.$el.checkbox((this.status === 'active' || this.active == true)  ? 'check' : 'uncheck');
             ...
             renderMenu : function(){
                this.$menu.empty();
                 if(this.isDisabled()){
                    ...
                    this.$menu.append('<div style="padding: 5px; width: 220px;">Click to Make Active/Inactive</div>');
            	 }
             }
